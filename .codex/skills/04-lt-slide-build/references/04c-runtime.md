...
- v3は要点を主表示し、hybridの話す内容は閉じた発話例として開閉できるようにする。空の接続欄を埋めない。旧v2は `話す内容` を主表示領域にし、`橋渡し`・`指差し`・`次の一言` は下段の補助領域へ置く。長い橋渡しが主台本の高さを奪うレイアウトは禁止する。
...
- 同一スライド・同一ノートの間は 主表示のDOM、発話例の開閉状態、`scrollTop` を保持する。step変更、投影側DOM同期、タイマー更新で先頭へ戻さない。
...
5. `S` で発表者ビューを開き、現在・次スライド、phaseの問い、ページの目的、選択形式のノート、reader context、bridge、タイマー、ショートカット一覧、双方向同期を確認する。長い `話す内容` を途中までスクロールし、タイマーが2秒以上進んでも位置が保持されることを確認する。
...
- 次プレビューは常に最終状態で表示される
- 発表者ビューで phaseの問い、ページの目的、選択形式のノート、reader context、bridge が読め、後からページ間の論理と実際の話し方を復元できる
