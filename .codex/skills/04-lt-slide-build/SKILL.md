...
1. ルートの `01-story.yaml` が単発かシリーズかを確認する。単発は `.lt-slide-work/02-blueprint.yaml` と `.lt-slide-work/visuals-manifest.yaml`、シリーズは各パートの指定ファイルを検証する。`presenter.include: true` のパートでは `data_file` の `presenter.json` を読み、表示名、bio、全links、avatar/qrの `use`・`path`、QRラベルを先に検証する。Storyに `design_system` があればregistryから同じID/versionを解決し、見つからなければ内蔵テーマへfallbackせず中止する。必須画像が未解決なら先に解消する。各パートについて、`../01-lt-slide-story/scripts/validate_knowledge_contract.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../01-lt-slide-story/scripts/validate_semantic_clarity.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../01-lt-slide-story/scripts/validate_duration_floor.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../01-lt-slide-story/scripts/validate_explanation_depth.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../01-lt-slide-story/scripts/validate_talkability.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../01-lt-slide-story/scripts/validate_roadmap.py --story <part-01-story.yaml> --blueprint <blueprint_file>`、`../02-lt-slide-blueprint/scripts/validate_visual_plan.py --story <part-01-story.yaml> --blueprint <blueprint_file>` を実行し、知識、理解確認、可視出典、主語・行為者・変更対象、本編／補足、説明時間、台本、道筋、必須ビジュアル計画のいずれかを満たさなければビルドを中止する。
1a. `project.authoring_mode: section-faithful` では、ビルド前に `../01-lt-slide-story/scripts/validate_section_fidelity.py --manifest <source-sections.yaml> --story <part-01-story.yaml> --blueprint <blueprint_file>` を実行する。節の統合、talk trackの改変、rendererが描画しないキーへの `visible_text` 退避があれば中止する。
2. `references/04a-pages.md` の指示に従い、設計図から静的な `.slide` 群を作る。選択したdesign-system tokenをCSS custom propertiesへ解決し、HTMLへ `data-design-system-id` と `data-design-system-version` を置く。`full-equivalence` では各slideへ `data-source-unit-ids`、`section-faithful` では各節スライドへ `data-source-section-ids` を置く。ここではページ送り、発表者ビュー、複雑なアニメーション制御を作り込まない。
...
- ブランドバッジは `.zone.brand-badge` として絶対配置し、スライド内で上下左右16px以上の専用safe areaを確保する。ブランド要素を余白検査から全面除外しない。
- card、flow node、code frame、根拠ラベル、結論帯などの本文surfaceは、親子包含を除き兄弟surface同士を交差させない。枠線が別surfaceのテキストを横切る場合は不合格とする。
...
- live本編枚数は `target_slide_count` と一致させる。時間別の枚数範囲は警告目安とし、`time_budget`、説明密度、問いへの答え、代表例、必要な実演、完了条件が妥当なら目安未満を許容する。appendix/referenceをlive枚数や登壇時間へ加算しない。
...
- `knowledge_contract_version: 1` の各デッキで `validate_knowledge_contract.py --story ... --blueprint ... --html ...` が成功し、意味ID、本編／補足、理解確認、可視出典が一致する
- `semantic_clarity_version: 1` の各デッキで `validate_semantic_clarity.py --story ... --blueprint ... --html ...` が成功し、title、message、動作を述べる本文の主語・行為者・変更対象・述語が可視のまま一致する
- `authoring_mode: section-faithful` の各デッキで `validate_section_fidelity.py --manifest ... --story ... --blueprint ... --html ...` が成功し、一節一枚以上の対応、Spoken Note先行のtalk track、実DOMの `visible_text` を確認する
...
- 全スライドでブランドバッジが16pxの専用safe areaへ収まり、本文surface同士の矩形交差とsurface内部overflowがない
