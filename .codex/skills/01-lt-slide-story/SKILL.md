...
13. `scripts/validate_spoken_notes.py --story <01-story.yaml>` と `scripts/validate_duration_floor.py --story <01-story.yaml>` を実行する。失敗時は成果物を次工程へ渡さず、代表例、比較、演習、デモ、完了条件を追加するか、発表時間を短縮する。
...
- `spoken_note` は各ページ固有の三行形式にする。`橋渡し: <前ページから進む理由>`、`読み方: <タイトルを含め、画面上の具体物をどう読むか>`、`次の判断: <聴衆が次に取る判断または行動>` を順に書く。仮文言、全ページ共通の文、画面の単純な復唱を使わない。
- 数値や最新情報には出典と確認日を残す。
...
- `validate_spoken_notes.py --story <01-story.yaml>` が成功する。各ノートは三行すべてを持ち、スライド固有のタイトルを読み方に含み、完全重複や仮ノートを含まない。
