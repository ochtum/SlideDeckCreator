...
完成済みの記事をスライド化するとき、発表向けの再構成によって節ごとの主張、手順、具体物が失われることを防ぐ。`section-faithful` では記事の見出し構造と順序を入力契約にし、各節の話す部分・画面に任せる部分を先に決め、配置後にノートを調整する。
    ...
    talk_track:
      ...
      beats:
        - point_id: section-004-p01
          delivery: spoken # v3。画面で伝わるbeatはvisualにし、spoken_textを空にする
          spoken_text: "既存システムの知識は、一つの巨大資料ではなく役割の異なる四つのドキュメントへ分けます。"
          visible_text: "四つのドキュメントを役割で分ける"
        - point_id: section-004-p02
          delivery: spoken # v3。画面で伝わるbeatはvisualにし、spoken_textを空にする
          spoken_text: "同じ説明を複製せず、更新する更新先を一つに決めます。"
          visible_text: "説明を複製せず更新先を一つにする"
```

v3は `delivery: spoken` のbeatだけをspeaker_cueのcuesまたはscriptへ含める。`delivery: visual` はvisible_textだけで伝え、台詞を足さない。spoken_noteは `speaker_notes.py` で生成する。旧v2では従来どおり全beatのspoken_textをscriptとノートへ含める。スライドのtitle、message、support、表、コード、図ラベルは `visible_text` または同じsectionのartifactから作る。入力にない新しい中心主張を投影面へ追加しない。
...
- `coverage: full`: 節のessential pointを、発話または表示で省略せず伝える。
...
シリーズ化する場合もsource sectionを分割管理の参照元にする。各partのStoryへ `section_scope` を置き、そのpartが担当する `section_id` を原文順で列挙する。
...
```powershell
python .codex/skills/01-lt-slide-story/scripts/validate_section_fidelity.py `
