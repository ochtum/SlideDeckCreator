...
- 本文・本編・各回・デッキ全体の最低枚数、目標枚数、固定枚数を設定しない。過去資料の物理枚数は `Evidence Sources` の観測値に限定し、今後の `target_slide_count` へ変換しない。
- 比率、連続数、最低間隔を必要に応じて設定する。
...
- 長時間資料がある場合も、style profileは本文総数を決めない。密度、低密度ページ比率、反復上限だけを渡し、各回の枚数は01の内容設計へ委ねる。
