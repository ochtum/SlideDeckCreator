...
function validateContracts(args) {
  ...
  const knowledge = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_knowledge_contract.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint, "--html", path.resolve(args.html)],
  );
  const semanticClarity = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_semantic_clarity.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint, "--html", path.resolve(args.html)],
  );
  const spoken = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_spoken_notes.py"),
    ["--story", paths.story, "--html", path.resolve(args.html)],
  );
  const talkability = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_talkability.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint, "--html", path.resolve(args.html)],
  );
  const visual = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "02-lt-slide-blueprint", "scripts", "validate_visual_plan.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint],
  );
  const depth = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_explanation_depth.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint, "--html", path.resolve(args.html)],
  );
  const roadmap = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_roadmap.py"),
    ["--story", paths.story, "--blueprint", paths.blueprint, "--html", path.resolve(args.html)],
  );
  ...
  const sectionFidelity = projectSelection.authoring_mode === "section-faithful"
    ? (fs.existsSync(sectionManifest)
      ? runPythonCheck(
          python,
          path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "validate_section_fidelity.py"),
  ...
  const motion = runPythonCheck(
    python,
    path.join(paths.root, ".github", "skills", "04-lt-slide-build", "scripts", "validate_animation_choreography.py"),
  ...
  const presenterBinding = presenterIncluded
    ? runPythonCheck(
        python,
        path.join(paths.root, ".github", "skills", "04-lt-slide-build", "scripts", "validate_presenter_binding.py"),
        [
          "--presenter",
  ...
  if (designSystemSelection.registry) {
    ...
    designSystem = runPythonCheck(
      python,
      path.join(paths.root, ".github", "skills", "07-lt-design-system-manager", "scripts", "manage_design_system.py"),
  ...
  if (fs.existsSync(rootStory) && fs.existsSync(sourceInventory)) {
    ...
    contentEquivalence = runPythonCheck(
      python,
      path.join(paths.root, ".github", "skills", "01-lt-slide-story", "scripts", "audit_content_equivalence.py"),
      contentArgs,
    );
  }
  checks.push(
