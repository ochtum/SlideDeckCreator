...
```powershell
& $node .github\skills\05-lt-slide-review\scripts\review_deck.js output\index.html --story .lt-slide-work\01-story.yaml --blueprint .lt-slide-work\02-blueprint.yaml --out .lt-slide-work\review --no-fail
```
...
- `.zone[data-zone]` 同士に加え、card、flow node、code frame、根拠ラベル、結論帯など意味のある本文surface同士の交差を検出する。親子包含、`data-overlap-ok`、背景、コネクタ、フッター、ページ番号は重なり検査から除外する。枠線が別surfaceのテキストを横切る状態は、テキスト自体のoverflowがなくても `content-surface-overlap` として不合格にする。
- ブランドバッジは既定16pxの専用safe areaへ完全に収める。装飾扱いで一般40px余白から除外しても、スライド端へ接触または越境した場合は `brand-safe-area` として不合格にする。
...
- dual-useのレビュー報告には、本編／補足枚数、live時間、補足へ移した内容、削除と理由、外部資料で更新した内容、未検証事項、前提、未対応知識、理解確認問題、可視出典の結果を含める。
...
- 一般40px余白の例外はフッター、ページ番号、背景装飾とする。ブランドバッジには一般余白の代わりに16pxの専用safe areaを必ず適用する。
